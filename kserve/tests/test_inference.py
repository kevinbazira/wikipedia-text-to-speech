...
it — the logic that the v1 fixes (defensive copy, short-chunk guard, drift
...
from inference import FADE_LEN, MAX_SEGMENT_CHARS, TTSInferencePipeline  # noqa: E402
...
class FakeAligner:
    ...
    real aligner does — the pipeline is responsible for offsetting them by the
    accumulated clock.
    """
...
def test_timestamps_never_exceed_total_duration():
    ...
    this is the regression guard for the original drift bug."""
    n1, n2, n3 = 1000, 1100, 900
    pipe = _make_pipeline(
        {"a": n1, "b": n2, "c": n3},
        {
...
def test_max_segment_chars_constant_exposed():
    """server.py imports this constant; guard against accidental removal."""
