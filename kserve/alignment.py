...
Extracted from ``wiki_tts/timestamps.py`` and packaged as a standalone
...
class Aligner:
    ...
    def __init__(self, model_dir: str):
        ...
        if not self.model_path.exists() or not self.processor_path.exists():
            raise FileNotFoundError(f"Wav2Vec2 model or processor not found at {model_dir}")
    ...
    def align(self, audio: np.ndarray, sample_rate: int, text: str) -> list[dict]:
        """Resample → inference → CTC decode → per-word timestamps.

        Returns a list of ``{"word": str, "start_ms": float, "end_ms": float}``
        or an empty list if the text is empty or alignment fails.
        """
        if not text.strip():
            return []
...
def _assign_frames_to_words(
    ...
    while word_idx < len(words):
        ...
        word_timestamps.append({"word": words[word_idx], "start_ms": start_ms, "end_ms": final_ms})
        word_idx += 1
