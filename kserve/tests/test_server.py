...
def _model():
    ...
    return WikipediaTTSModel.__new__(WikipediaTTSModel)
...
def test_postprocess_passes_timestamps_through():
    ...
    out = m.postprocess({"audio": np.zeros(10, dtype=np.float32), "sample_rate": SAMPLE_RATE, "timestamps": ts})
...
def test_predict_awaits_and_returns_pipeline_result():
    ...
    class FakePipeline:
        def predict(self, segments, default_voice, default_speed, default_lang):
            return {"audio": np.zeros(3, dtype=np.float32), "sample_rate": SAMPLE_RATE, "timestamps": []}
...
def test_predict_runs_off_the_event_loop_thread():
    ...
    class ThreadCapturingPipeline:
        def predict(self, *a, **k):
            ...
            return {"audio": np.zeros(1, dtype=np.float32), "sample_rate": SAMPLE_RATE, "timestamps": []}
